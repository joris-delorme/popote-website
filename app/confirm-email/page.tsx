...
export default async function page({ searchParams }: { searchParams: { email: string, name: string } }) {
    ...
    if (error) {
        ...
        return (
            <>
                <h1 className="font-serif text-xl font-black mb-2">{searchParams.name}, une erreur c&apos;est produite...</h1>
                <p className="text-sm mb-6">Aïe aïe aïe... Il semble que nous avons rencontré un problème lors de votre inscription à notre newsletter avec {searchParams.email}.</p>
                ...
                <p className="text-muted-foreground text-sm mt-10">
                    Pas de panique, si l&apos;erreur perciste, nous avons déjçà était prévenu et nous mettons tout en oeuvre pour que cette erreur disparaisse...
                    <br />
                    <br />
                    En attendant vous pouvez découvrire <Link className="underline" href="/#recipesOfTheWeek">les recettes de la semaines</Link>.
                </p>
            </>
        )
    }
    ...
    return (
        <div className="">
            ...
            <ol className="list-disc text-sm list-inside gap-2 grid text-muted-foreground">
                <li>Veulent faire dès gâteaux au yaourt sans avoir de yaourts </li>
                ...
                <li>Et pliens d&apos;autre truc autours de la cuisine !</li>
            </ol>
            <br />
            <p className="font-medium text-center">C&apos;est pour vous et toutes ces personnes que j&apos;ai créer Popote.</p>
